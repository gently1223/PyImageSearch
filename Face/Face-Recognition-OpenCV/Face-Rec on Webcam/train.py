...
def trainRecognizer(db_folder, trainSize=config.DEFAULT_FACE_SIZE, showFaces=False, forceTrain=False):
    ...
    for folder in folders:
        ...
        if showFaces:
            #cv2.namedWindow("Training in process", 1)
            cv2.imshow("Training recognizer, press enter after each subset...", combineFaces(faces))
            cv2.waitKey(0)

    #if showFaces:
        #cv2.destroyWindow("Training in process")
